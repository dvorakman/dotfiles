...
export function launchCustomCommand(command) {
    ...
    if (args[0] == '>raw') { // Mouse raw input
        ...
        execAsync([`bash`, `-c`, `${GLib.get_user_config_dir()}/ags/scripts/color_generation/switchwall.sh`, `&`]).catch(print)
    }
    else if (args[0] == '>color') { // Generate colorscheme from color picker
        execAsync([`bash`, `-c`, `${GLib.get_user_config_dir()}/ags/scripts/color_generation/switchcolor.sh --pick`, `&`]).catch(print)
    }
        ...
        execAsync([`bash`, `-c`, `mkdir -p ${GLib.get_user_cache_dir()}/ags/user && sed -i "1s/.*/light/"  ${GLib.get_user_cache_dir()}/ags/user/colormode.txt`])
            .then(execAsync(['bash', '-c', `${GLib.get_user_config_dir()}/ags/scripts/color_generation/switchcolor.sh`]))
            .catch(print)
    }
        ...
        execAsync([`bash`, `-c`, `mkdir -p ${GLib.get_user_cache_dir()}/ags/user && echo "material" > ${GLib.get_user_cache_dir()}/ags/user/colorbackend.txt`]).catch(print)
            .then(execAsync(['bash', '-c', `${GLib.get_user_config_dir()}/ags/scripts/color_generation/switchwall.sh --noswitch`]).catch(print))
            .catch(print)
    }
        ...
        console.log('Todo functionality not implemented in AGS v2 migration')
    }
